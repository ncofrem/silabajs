(function () {
  ...
  function posicionSilabas(palabra) {
    ...
    for (let actPos = 0; actPos < silaba.longitudPalabra;) {
      ...
      if ((encontroTonica) && (silaba.tonica == 0)) { silaba.tonica = silaba.numeroSilaba; } // Marca la silaba tónica
    }
  ...
  function ataque(pal, pos) {
    ...
    if (pos < silaba.longitudPalabra - 1) {
      if (pal[pos] == 'u') {
        ...
        if ((pal[pos] === 'ü') || (pal[pos] == 'Ü')) if (ultimaConsonante == 'g') pos++;
      }
    }
  ...
  function coda(pal, pos) {
    if ((pos >= silaba.longitudPalabra) || (!esConsonante(pal[pos]))) return pos; // No hay coda
  ...
  if (typeof window.silabaJS === 'undefined') { window.silabaJS = silabaJS; }
}());
